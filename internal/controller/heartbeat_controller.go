...
func (r *HeartbeatReconciler) processHeartbeat(
	...
	secret, err := r.processSecretStep(ctx, heartbeat, req, heartbeatLog)
	if err != nil {
		return err
	}

	// Step 3: Extract and validate the target endpoint
	targetEndpoint, err := r.processTargetEndpointStep(ctx, heartbeat, secret, heartbeatLog)
	if err != nil {
		return err
	}

	// Step 4: Extract report endpoints for health status reporting
	reportEndpointsSecret, err := r.processReportEndpointsStep(ctx, heartbeat, secret, heartbeatLog)
	if err != nil {
		return err
	}

	// Step 5: Perform health check and report status
	return r.processHealthCheckStep(ctx, heartbeat, targetEndpoint, reportEndpointsSecret, heartbeatLog)
}

// processSecretStep handles the secret fetching and validation step.
func (r *HeartbeatReconciler) processSecretStep(
	ctx context.Context,
	heartbeat *monitoringv1alpha1.Heartbeat,
	req ctrl.Request,
	heartbeatLog logr.Logger,
	...
	if err != nil {
		logger.Error(heartbeatLog, "Failed to fetch resource", err, map[string]interface{}{
	...
	logger.Info(heartbeatLog, "Secret fetched successfully", map[string]interface{}{
		"secret_namespace": heartbeat.Spec.EndpointsSecret.Namespace,
		"secret_name":      heartbeat.Spec.EndpointsSecret.Name,
	})
...
// processTargetEndpointStep handles the target endpoint extraction step.
func (r *HeartbeatReconciler) processTargetEndpointStep(
	ctx context.Context,
	heartbeat *monitoringv1alpha1.Heartbeat,
	...
	if err != nil {
		...
		logger.Error(heartbeatLog, "Failed to extract endpoint", err, fields)
		return "", err
	}

	logger.Info(heartbeatLog, "Endpoint extracted successfully", map[string]interface{}{
		"key":      heartbeat.Spec.EndpointsSecret.TargetEndpointKey,
...
// processReportEndpointsStep handles the report endpoints extraction step.
func (r *HeartbeatReconciler) processReportEndpointsStep(
	ctx context.Context,
	heartbeat *monitoringv1alpha1.Heartbeat,
	secret *corev1.Secret,
	heartbeatLog logr.Logger,
...
func (r *HeartbeatReconciler) extractSingleReportEndpoint(
	...
	endpoint, err := r.extractEndpointFromSecret(ctx, heartbeat, secret, key)
	if err != nil {
...
// processHealthCheckStep handles the health check and reporting step.
func (r *HeartbeatReconciler) processHealthCheckStep(
	ctx context.Context,
	heartbeat *monitoringv1alpha1.Heartbeat,
